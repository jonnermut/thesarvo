...
public class ImageReadNode extends ReadNode
{
	...
	private void setWidthAndLegend()
	{
		...
		if (legend )
		{
			...
			final List<SimpleModel> list = ((XmlSimpleModel)getModel()).getList("legend");
	...
	{
		private final List<SimpleModel> list;
		int r;
		private Document extraXml;
		
		private LegendPopulator(List<SimpleModel> list, int row)
		{
			this.list = list;
			r = row;
		}
