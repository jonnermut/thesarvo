...
import static com.thesarvo.xphone.client.util.StringUtil.notNull;
...
import com.thesarvo.xphone.client.model.xml.XmlRequestCallback;
...
public class Controller extends com.thesarvo.xphone.client.controller.ControllerImpl
{
	//private static final String CONFLUENCE_PLUGINS_SERVLET = "http://www.thesarvo.com/confluence/plugins/servlet/";
	...
	{
		String url = getServletUrl() + "guide/xml/" + getCurrentGuide().getGuideId();
		url += "?user=" + getUser();
		...
		XmlService.doXmlRequest(url, RequestBuilder.POST, data, new XmlRequestCallback()
		{
	...
	public void onCancel(NodeWrapper nw)
	{
		...
		else
		{
			...
			{
				nw.getNode().getParentNode().replaceChild(currentEditNodeClone, nw.getNode());
	...
	public String getAttachmentUrl(String src, boolean thumbnail, String width)
	{
		...
		{
			// TODO - hardcoded path
			ret = "http://www.thesarvo.com/confluence/plugins/servlet/guide/image/" + getGuideId() + "/" + src;
	...
	public void populateAttachments(final ListBox srcListBox)
	{
		...
		{
			String url = getServletUrl() + "guide/attachments/" + getCurrentGuide().getGuideId() ;
	...
	public void getGuideXml(final String pageId, final boolean cache, final boolean immediateIfCached, final XmlRequestCallback callback)
	{
		
		if (cache)
		{
			final Document ret = guideXmlCache.get(pageId);
			if (ret!=null)
			{
				
				if (immediateIfCached)
				{
					callback.handleXml(ret);
				}
				else
				{
		...
		if (Window.Location.getHost().contains("127.0.0.1") || Window.Location.getHost().startsWith("192.168")  || Window.Location.getHost().startsWith("172.27"))
		{
			url = "../data/MtBrown.xml";
		}
...
//		}
	...
	private void addClimbStringToList(List<Node> climbs, List<String[]> ret, String prefix)
	{
		for (Node n : climbs)
		{
...
//	}
	...
	private void addNodesToGuideView(final GuideView gv)
	{
		...
		DeferredCommand.addCommand(new Command()
		{
			...
			public void execute()
			{
				...
				if (isCanEdit() && getUser()!=null && getUser().equals("jnermut")  )
				{
					gv.showAdvanced();
				}				
			}
	...
	public String getGraphUrl()
	{
		...
		return getServletUrl() + "graph?type=png&pageId=" + getCurrentGuide().getGuideId();
	}
	...
	private void upgradeTo3()
	{
		...
		//Pattern pattern1 = new Pattern("([\\(]*\\d+[a-z]?[\\),\\.])\\s([\\d,\\(,\\),m,\\s]+)(\\.?)");
...
//			String[] splits = stop.split(text);
			...
			if (splits.length > 1)
			{
				...
				if ( ! (fa.contains("20") || fa.contains("19") || fa.contains("6") || fa.contains("7") || fa.contains("8") || fa.contains("9") || fa.contains("0") ) ) 
				{
					fa = null;
				}
			...
			if (fa!=null )
			{
				...
				{
					log("Exception trying to fix dates for: " + fa + " - " + e.getMessage());
				}
			...
			final SimpleDataBinder binder, final String extraPageId, final boolean insertBefore)	
	{
		...
		if (StringUtil.isNotEmpty(extraPageId))
		{
			XmlRequestCallback cb = new XmlRequestCallback()
			{
				...
				public void handleXml(Document xml)
				{
					// add the new climbs to the list
					List<String[]> extra = getClimbStrings(xml, extraPageId + ":");
					
					if (insertBefore)
						climbs.addAll(0, extra);
					else
						climbs.addAll( extra);
	...
	{
		return isMultiPage() && BrowserUtil.isMobileBrowser() && Window.Location.getProtocol().startsWith("file");
	}
	
	public GuideView createGuideView(String id)
	{
		return (GuideView) createView("GuideView", null, id);
	}
	
	@Override
	protected Widget createView(String type, ViewConfig viewConfig,
			final String entityId)
	{
		...
		if (ret==null)
		{
			if (type.equals("GuideView"))
			{
				
				final GuideView gv =  new GuideView(viewConfig, entityId); 
				ret = gv;
