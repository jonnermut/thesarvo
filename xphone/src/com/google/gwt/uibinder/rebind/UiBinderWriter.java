...
import com.thesarvo.xphone.rebind.XphoneParsers;
...
public class UiBinderWriter {
  ...
  public void addElementParser(String gwtClass, String parser) {
  ...
  private void registerParsers() {
    ...
    // added by JN for XPhone
    XphoneParsers.addXphoneParsers(this);
  }
  ...
  // XPhone Added code (JN)

  // this is all hacky as crap - hopefully will be able to remove GWT2.1 or GWT2.2
  
  //int statementIndex = -1;
  //int initIndex = -1;
  //List<String> newFields = new ArrayList<String>();
	
  List<String> savedStatements;
  List<String> savedInits;
  FieldManager savedFieldManager;
  
  public void startDelegation()
  {
	  savedStatements = statements;
	  savedInits = initStatements;
	  savedFieldManager = fieldManager;
	  
	  statements = new ArrayList<String>();
	  initStatements = new ArrayList<String>();
	  fieldManager = new FieldManager(oracle, logger);
	  
//	  statementIndex = statements.size();
//	  initIndex = initStatements.size();
//	  newFields.clear();
  }
  
  public FieldWriter lookupField(String name)
  {
	  return fieldManager.lookup(name);
  }
  
  public String endDelegation() throws UnableToCompleteException 
  {
	  StringWriter stringWriter = new StringWriter();
	  IndentedWriter niceWriter = new IndentedWriter(new PrintWriter(stringWriter));
	  
	  writeGwtFields(niceWriter);
	  writeAddedStatements(niceWriter);
	  writeInitStatements(niceWriter);
	  writeHandlers(niceWriter);

	  
	  statements = savedStatements;
	  initStatements = savedInits;
	  fieldManager = savedFieldManager;
